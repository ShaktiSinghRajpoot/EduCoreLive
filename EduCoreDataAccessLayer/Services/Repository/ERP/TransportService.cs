...
namespace EduCoreDataAccessLayer.Services.Repository.ERP
{
    public class TransportService : ITransportService
    {
        ...
        public async Task<List<TransportVehicle>> GetVehiclesAsync(int tenantId, int schoolId, int actionUserId)
        {
            ...
            foreach (DataRow r in ds.Tables[0].Rows)
            {
                list.Add(new TransportVehicle
                {
                    ...
                    DriverStaffId = r.Table.Columns.Contains("driver_staff_id") && r["driver_staff_id"] != DBNull.Value ? ToInt(r["driver_staff_id"]) : (int?)null,
        ...
        public async Task<List<TransportDriverOption>> GetDriversAsync(int tenantId, int schoolId, int actionUserId)
        {
            var list = new List<TransportDriverOption>();
            var ds = await ExecAsync(SpVehicle, VehicleParams("GetDrivers", tenantId, schoolId, actionUserId));
            if (ds.Tables.Count == 0) return list;
            foreach (DataRow r in ds.Tables[0].Rows)
            {
                list.Add(new TransportDriverOption
                {
                    StaffId     = ToInt(r["staff_id"]),
                    FullName    = r["full_name"]?.ToString() ?? string.Empty,
                    Mobile      = r["mobile"] == DBNull.Value ? null : r["mobile"].ToString(),
                    Designation = r["designation"] == DBNull.Value ? null : r["designation"].ToString()
                });
            }
            return list;
        }
        ...
        public async Task<(bool, string)> SaveVehicleAsync(TransportVehicle v, int tenantId, int schoolId, int actionUserId)
        {
            ...
            p[10].Value = v.DriverStaffId.HasValue && v.DriverStaffId > 0 ? v.DriverStaffId.Value : DBNull.Value;
            try
            {
        ...
        private static NpgsqlParameter[] VehicleParams(string op, int t, int s, int u) => new NpgsqlParameter[]
        {
            ...
            new("p_route_id",        NpgsqlDbType.Integer) { Value = DBNull.Value },
            new("p_driver_staff_id", NpgsqlDbType.Integer) { Value = DBNull.Value },
            new("p_result", NpgsqlDbType.Refcursor) { Direction = ParameterDirection.InputOutput, Value = "result_cursor" }
        };
