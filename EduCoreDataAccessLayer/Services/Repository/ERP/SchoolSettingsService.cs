...
namespace EduCoreDataAccessLayer.Services.Repository.ERP
{
    public class SchoolSettingsService : ISchoolSettingsService
    {
                    ...
                    if (!string.IsNullOrWhiteSpace(className))
                    {
                        ...
                        if (detail == null)
                        {
                            detail = new AcademicClassDetail
                            {
                                ...
                                CoordinatorStaffId = AsNullableInt(row, "coordinator_staff_id")
                            };
        ...
        public async Task<FeeHead> GetFeeHeadPageAsync(FeeHead query, int tenantId, int schoolId, int actionUserId)
        {
            query.Items = new List<FeeHead>();
            query.TotalCount = 0;
            ...
            var parameters = new NpgsqlParameter[]
            {
                ...
                new NpgsqlParameter("p_school_id", schoolId),
                new NpgsqlParameter("p_action_user_id", actionUserId),
                new NpgsqlParameter("p_search", (object?)query.Search ?? DBNull.Value),
                new NpgsqlParameter("p_frequency", (object?)query.FilterFrequency ?? DBNull.Value),
                new NpgsqlParameter("p_page_no", query.Page),
                new NpgsqlParameter("p_page_size", query.PageSize),
                new NpgsqlParameter("p_sort_column", (object?)query.SortColumn ?? DBNull.Value),
                new NpgsqlParameter("p_sort_dir", query.SortDirection),
                new NpgsqlParameter("p_result", NpgsqlDbType.Refcursor) { Direction = ParameterDirection.InputOutput, Value = "fee_head_list_cursor" }
            };

            var ds = await _db.ExecuteProcedureWithCursorsAsync("core.sp_fee_head_list", parameters);
            ...
            {
                query.Items.Add(new FeeHead
                {
                    Operation       = "SaveFeeHead",
                    TenantId        = row["tenant_id"] == DBNull.Value ? tenantId : Convert.ToInt32(row["tenant_id"]),
                    SchoolId        = row["school_id"] == DBNull.Value ? schoolId : Convert.ToInt32(row["school_id"]),
                    FeeHeadId       = row["fee_head_id"] == DBNull.Value ? 0 : Convert.ToInt32(row["fee_head_id"]),
                    FeeHeadName     = row["fee_head_name"] == DBNull.Value ? string.Empty : row["fee_head_name"].ToString() ?? string.Empty,
                    Frequency       = row["frequency"] == DBNull.Value ? string.Empty : row["frequency"].ToString() ?? string.Empty,
                    DefaultAmount   = row["default_amount"] == DBNull.Value ? 0 : Convert.ToDecimal(row["default_amount"]),
                    FeeType         = row["fee_type"] == DBNull.Value ? string.Empty : row["fee_type"].ToString() ?? string.Empty,
            ...
            query.TotalCount = table.Columns.Contains("total_count") && first["total_count"] != DBNull.Value
                ? Convert.ToInt32(first["total_count"]) : 0;
            query.MandatoryCount = table.Columns.Contains("mandatory_count") && first["mandatory_count"] != DBNull.Value
                ? Convert.ToInt32(first["mandatory_count"]) : 0;
            query.OptionalCount = table.Columns.Contains("optional_count") && first["optional_count"] != DBNull.Value
                ? Convert.ToInt32(first["optional_count"]) : 0;
