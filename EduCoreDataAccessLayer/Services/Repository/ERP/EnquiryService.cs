...
namespace EduCoreDataAccessLayer.Services.Repository.ERP
{
    public class EnquiryService : IEnquiryService
    {
        ...
        private static void FillBaseFields(EnquiryModel m, DataRow row, int tenantId, int schoolId)
        {
            ...
            // Legacy fallback: older enquiries stored the guardian under parent_name
            // only (before father/mother were split out). Fold it into FatherName so
            // no data is lost now that the standalone ParentName field is retired.
            if (string.IsNullOrWhiteSpace(m.FatherName))
                m.FatherName        = NullStr(row, Has(row, "parent_name") ? "parent_name" : "derived_parent_name");
