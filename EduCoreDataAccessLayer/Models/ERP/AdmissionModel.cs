namespace EduCoreDataAccessLayer.Models.ERP
{
    ...
    // ── One model, many jobs ───────────────────────────────────────────
    //   • one grid row  → StudentId, StudentName, ClassName …
    //   • the listing    → Items (the current page of rows)
    //   • list filters   → FilterClass/Gender/Fee/Year + Search (base)
    //   • paging + sort  → Page, PageSize, SortColumn … (from ListModelBase)
    //   • dropdowns      → ClassList, YearList (filter-bar selects)
    // ListModelBase lives in the parent namespace (EduCoreDataAccessLayer.Models),
    // resolved without a using because this file's namespace is a child of it.
    public class StudentListModel : ListModelBase
    {
        ...
        // ── List filters (distinct from any row/form field above) ─
        public string? FilterClass  { get; set; }
        public string? FilterGender { get; set; }
        public string? FilterFee    { get; set; }   // Paid | Partial | Pending
        public string? FilterYear   { get; set; }

        // ── Summary tiles (computed over the whole filtered set) ──
        public int MaleCount   { get; set; }
        public int FemaleCount { get; set; }
        public int ActiveCount { get; set; }

        // ── Filter-bar dropdowns ─────────────────────────────────
        public List<Microsoft.AspNetCore.Mvc.Rendering.SelectListItem> ClassList { get; set; } = new();
        public List<Microsoft.AspNetCore.Mvc.Rendering.SelectListItem> YearList  { get; set; } = new();
    }
}
